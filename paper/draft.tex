...
(iii)~\emph{Certification:} a whole-pass stability certificate is
constructed from exponential delay-independent Lyapunov--Krasovskii
point certificates chained by jump factors and a dwell-time rule.  A
quantitative obstruction is identified---parameter variation rotates
the razor-thin modal eigendirections by many linewidths, so a common
quadratic storage per practical cell cannot exist (feasible cell
diameter scales with $\zeta$)---and the certificate is therefore taken
to its thin-cell limit with \emph{solver-free} point certificates built
on the stable invariant subspace of a bounded-real Hamiltonian; en
route, recorded SDP infeasibility verdicts at single vertices are shown
to be solver failures.  (iv)~\emph{Results:} the realizable member is
...
\item A whole-pass certificate (Section~\ref{sec:pass}): exponential
delay-independent Lyapunov--Krasovskii (LK) point certificates, jump
factors between neighbouring functionals, and a dwell rule giving a
certified traversal speed $\vcert$.  We identify the quantitative
obstruction to cell-wise common storage (modal-direction rotation
vs.\ linewidth), build solver-free point certificates on the
bounded-real Hamiltonian, document recorded SDP verdicts as solver
false-negatives, and formalise the thin-cell limit as a differential
theorem whose hypotheses (single frozen construction regime, pointwise
verified margins, converged derivative cost $\sigma(x)$) are all
measured (Fig.~\ref{fig:refine},~\ref{fig:sigma}).
...
\label{eq:lk}
\end{equation}
Feasibility at $(\alpha, \tau)$ covers every delay
...
\subsection{The obstruction, measured: rotation vs.\ linewidth}
...
delay-disc measure $\|G\|_\infty$ of the scalar channel: no phase is
lost between the crossing test and the DI premise, and per vertex the
LK form is KYP-equivalent to $\|G\|_\infty < 1$.  Measured on a refused
...
law.}  The vertex spread inside any practical cell rotates the
...
interval, let fixed construction parameters give
$P(x), Q(x)$ from \eqref{eq:ric} (smooth wherever the Hamiltonian
retains its dichotomy) such that (i) the pointwise LMI \eqref{eq:lk}
is verified at every $x$; (ii)
$\sigma(x) = \max\big(\|P^{-1/2}P'P^{-1/2}\|,
\|Q^{-1/2}Q'Q^{-1/2}\|\big)$; and (iii) regime boundaries carry jump
factors $\mu_b$.  Then $V$ of \eqref{eq:lk} satisfies
$\dot V \le (-2\alpha + v\,\sigma(x))\,V$ inside regimes and
$V^{+} \le \mu_b V$ at boundaries, so the whole pass is exponentially
stable whenever $v \le \min_x 2\alpha/\sigma(x)$ and the dwell rule
holds at each boundary.
\end{theorem}

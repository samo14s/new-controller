...
(i)~\emph{Pricing:} the robust-stability gate of the benchmark is shown
to be dominated by its uncertainty \emph{reading}, not its physics: on a
plate with $\zeta\approx0.3\%$, treating real parametric perturbations
as complex lets stiffness uncertainty masquerade as negative damping and
overprices the gate by a factor of 4--8 across the whole scheduling
domain.  A mixed real/complex $\mu$ re-reading
(Fan--Tits--Doyle bound with solver-independent numerical verification
of the returned scales) opens the gate at every one of 21 scheduling
nodes for both the benchmark's own controller (peak
$\muRS=0.419$) and a realizable scheduled member ($0.383$), where the
complex reading had reported $1.16$--$85.8$.
(ii)~\emph{Structure:} an actuator-aware scheduled member is
synthesised with its command filter \emph{inside} the design model
(augmented LQR in scaled coordinates), and three measured structural
ceilings are reported, including the finding that the unfiltered
...
(iii)~\emph{Theory:} a whole-pass stability theory is developed and
\emph{proved}: a delay-cover proposition (one certificate covers every
shorter delay, hence all faster spindle speeds), an elementary
frequency-domain equivalence between the delay-independent LK
inequality and the $H_\infty$ contract of the rank-one regenerative
channel, a finite-cell chain theorem (cells, jump factors, dwell rule
$\Rightarrow$ exponential stability of the traversal with an explicit
certified feed speed), and its differential (adiabatic) limit for
$C^1$ certificate fields.  Complete proofs are given; every hypothesis
is additionally \emph{verified computationally} on the benchmark.  A
quantitative empirical law is identified en route---parameter variation
rotates the razor-thin modal eigendirections by many linewidths, so a
common quadratic storage per practical cell cannot exist (feasible cell
diameter scales with $\zeta$)---which motivates the thin-cell limit and
its \emph{solver-free} point certificates built on the stable invariant
subspace of a bounded-real Hamiltonian; recorded SDP infeasibility
verdicts at single vertices are shown to be solver failures.  (iv)~\emph{Results:} the realizable member is
certified over the full parameter box at the reference feed with
$\vcert = 79.9$--$82.7$\,mm/s ($16\times$ the actual feed of
4.90\,mm/s), and with the removal schedule moving,
$17.8$\,mm/s ($3.6\times$); the filtered gate-crossing member is
certified at full box down to $h=0.104$\,mm cell size at
$\ap=0.15$\,mm.  Grid refinement exposes and corrects an
over-optimistic coarse-grid claim at $\ap=0.28$\,mm, and
delay-exploiting (TDC-type) members are classified out of the
delay-independent contract by design.  Every number is derived from
stored, re-checkable certificate matrices.
\end{abstract}
...
\begin{enumerate}
\item A mixed real/complex $\mu$ re-reading of the robust-stability
gate (Section~\ref{sec:readings}), with a numerical-verification
protocol that removes the SDP solver from the trust chain.  Measured on
the full 21-node scheduling grid, the complex reading overprices the
gate by $4$--$8\times$; the gate is in fact met at every node by the
benchmark controller itself and by a realizable scheduled member
(Fig.~\ref{fig:sweep}).
\item An actuator-aware scheduled controller family whose command
filter lives inside the design model (augmented LQR solved in scaled
coordinates), together with three measured structural results: a
performance ceiling $J\approx-0.28$ for filtered structures (its named
challenger, phase-lead compensation, is tested and rejected---the
optimiser parks the lead at its lower bounds); the certainty-equivalence
failure of the series-filter variant; and the finding that the
baseline scheduled-LQG performance rests on parasitic MHz compensator
poles (Section~\ref{sec:member}).
\item A whole-pass stability \emph{theory} with complete proofs
(Section~\ref{sec:pass}, Appendix~\ref{app:proofs}):
Proposition~\ref{prop:cover} (delay cover), Proposition~\ref{prop:kyp}
(rank-one KYP equivalence, by an elementary frequency-domain argument),
Theorem~\ref{thm:chain} (finite-cell chain: cells + jump factors +
dwell rule $\Rightarrow$ exponential whole-pass stability with the
explicit certified traversal speed $\vcert$ of \eqref{eq:vcert}), and
Theorem~\ref{thm:diff} (the differential/adiabatic limit for $C^1$
certificate fields).  Alongside the proofs, every hypothesis is
verified computationally on the benchmark (single frozen construction
regime, pointwise margins at $100\%$ of nodes, derivative cost
$\sigma(x)$ converged under grid halving;
Fig.~\ref{fig:refine},~\ref{fig:sigma}).  We further identify the
quantitative \emph{empirical} obstruction to cell-wise common storage
(modal-direction rotation vs.\ linewidth), build solver-free point
certificates on the bounded-real Hamiltonian, and document recorded
SDP verdicts as solver false-negatives.
\item An honesty layer that we consider part of the method: grid
refinement is used to \emph{falsify} an initially certified
coarse-grid claim (the $\ap=0.28$\,mm full-box certificate of the
filtered member), the divergence mechanism is named, and
delay-exploiting TDC members are classified out of the
delay-independent contract by design rather than reported as failures.
\end{enumerate}
...
$\mu$-synthesis loop with time-delay control \cite{Du2024}.  In all of
these, stability is assessed at frozen operating points (analytically
or experimentally); certified statements over an \emph{entire pass},
and mixed real/complex $\mu$ readings of the robustness gate, appear
to be absent from this literature.
...
$f = (540, 1068, 2787, 3351, 4122)$\,Hz with damping ratios
$\zeta = (0.31, 0.17, 0.27, 0.56, 0.35)\times10^{-2}$; a piezoelectric
...
unfiltered member ($+0.138$) that fails the gate at $3.8$.  The one
...
($J = +0.138$) carries compensator poles at up to $8.8$\,MHz (family
...
$J = -0.482$ with two agreeing seeds.  The gap of $0.62$ in the shared
...
rate $\alpha$ for every delay $\tau' \in [0, \tau]$.
\end{proposition}
...
refinement)---$16.9\times$ the actual feed.  The $\zeta$-slice rows
...
definite matrices satisfying the pointwise LMI \eqref{eq:lk} with rate
$\alpha$ (in our construction: the frozen-parameter Riccati field of
\eqref{eq:ric}, which is real-analytic wherever the Hamiltonian
retains its dichotomy \cite{Lancaster1995}).  Define
...
boundary.  (Proof in Appendix~\ref{app:proofs}.)
\end{theorem}
...
recomputable from stored matrices).}
\label{tab:final}
\begin{tabular}{@{}llll@{}}
\toprule
Quantity & Value & Margin & Basis \\
\midrule
...
Realizability price (sched.\ LQG) & $\Delta J = 0.62$ & --- &
$+0.138$ (8.8\,MHz poles) vs $-0.482$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:final} collects the headline numbers.  Under the
corrected pricing the spatio-temporal variation is no longer an
adversary to be over-insured against: it is measured, scheduled,
sliced where constant, chained where moving, and the guarantee runs
over the entire operating domain with an explicit rate margin against
the actual feed.
...
\texttt{adiab\_diff.npz}, run logs), and corrections to earlier
readings are appended to the record rather than rewritten.
...
\emph{In-cell decay.}  $\Phi$ is affine in $(A, A_d)$, so negativity
at the vertex family implies negativity on its convex hull, and
Proposition~\ref{lem:sproc} extends it to the measured
$\varepsilon_k$-residual around the interpolant; hence for every
admissible parameter path, $\dot V_k + 2\alpha V_k \le 0$ inside cell
$k$, uniformly in the delay $\tau' \le \tau$ by

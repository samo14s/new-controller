% Draft manuscript -- Elsevier elsarticle class, numbered references
% (pdflatex; the bibliography is inline thebibliography).
\documentclass[preprint,12pt]{elsarticle}
\usepackage{amsmath,amssymb}
% elsarticle pre-defines a proof environment; amsthm's replaces it
\let\proof\relax
\let\endproof\relax
\usepackage{amsthm}
\usepackage{graphicx}
...
\title{Parameter-scheduled active chatter control of thin-walled
milling: a mixed-$\mu$ full-domain gate and an adiabatic whole-pass
stability certificate}

% [Author list and affiliations to be finalised.]
\author[enis]{Sami Soufi}
\ead{sami.soufi@enis.tn}
\affiliation[enis]{organization={National Engineering School of Sfax
(ENIS)}, city={Sfax}, country={Tunisia}}
...
\begin{keyword}
thin-walled milling \sep chatter \sep active vibration control
\sep parameter scheduling \sep time-delay systems \sep
Lyapunov--Krasovskii certificates \sep mixed $\mu$-analysis \sep
stability lobes
\end{keyword}
...
\begin{align*}
f &= (540,\, 1068,\, 2787,\, 3351,\, 4122)\ \mathrm{Hz},\\
...
\begin{tabular}{@{}p{3.6cm}p{3.3cm}p{1.6cm}p{4.0cm}@{}}
\toprule
...
\label{tab:baseline}
\begin{tabular}{@{}p{3.9cm}p{3.6cm}p{4.6cm}@{}}
\toprule
...
\label{tab:head2head}
\begin{tabular}{@{}p{3.9cm}p{2.9cm}p{3.0cm}p{2.7cm}@{}}
\toprule
...
validity scale of the point chain.  Its crossing pair completes the
row to the stage-5/6 standard: $\ap^{\infty} = 0.351$\,mm and
$\delta_{\max} = 1.215$ (r5: $0.284$\,mm and $0.918$), and at the
reference depth its frozen-point crossing peak over the full vertex
family is $0.827 < 1$ with $\tau_{\max}$ unbounded---a delay-covered
pointwise statement r5 could not make (its peak was $1.066$); the
classical common-$P$ LK at $\ap^{\infty}$ remains infeasible, as the
$\zeta$-law predicts.
...
exposure the gate exists to police.  Its crossing pair, re-measured
fresh and matching the stored stage-5/6 record digit for digit, is
$\ap^{\infty} = 0.397$\,mm / $\delta_{\max} = 1.457$ with a common-$P$
LK at $\ap^{\infty}$: just \emph{below} the benchmark's
$0.436$/$1.543$ on the pointwise crossing axis whose lobe axis it
dominates---the two delay-exploiting members are ranked oppositely by
the two metrics, a reminder that the frozen crossing certificate and
the Floquet lobe are different questions.  Scheduling the delayed
pair on
...
\label{tab:lobefirst}
\begin{tabular}{@{}p{2.7cm}p{2.3cm}p{2.4cm}p{2.5cm}p{2.5cm}@{}}
\toprule
...
crossing $\ap^{\infty}$ / $\delta_{\max}$ & $0.436$\,mm / $1.543$ &
$0.284$\,mm / $0.918$ & $0.351$\,mm / $1.215$ & $0.397$\,mm /
$1.457$ \\
...
(7)~The lobe-first champion's certified whole-pass depth remains
$0.15$\,mm, not the reference depth; its reference-depth crossing
statement (peak $0.827$, $\tau$-covered) is pointwise, not a
...
\texttt{mu\_real\_l21.npz}, \texttt{cross\_l.npz},
\texttt{cross\_ps\_tdc\_j.npz}, run logs), and
...
\bibitem{Sobhanipour2023} H.~Sobhanipour, B.~Rezaie, Enhanced
exponential
...
Trans.\ Syst.\ Man Cybern.\ Syst.\ 53 (2023).
\bibitem{Zhao2013} X.~Zhao, L.~Zhang, P.~Shi, Stability of a class
of switched positive linear time-delay systems, Int.\ J.\ Robust
Nonlinear Control 23 (2013) 578--589.
...
with time-varying delay, Automatica 175 (2025) 112183.
...
\bibitem{Du2022} J.~Du, X.~Long, Chatter suppression for milling of
thin-walled workpieces based on active modal control, J.\ Manuf.\
Process.\ 84 (2022) 1042--1053.
...
stack actuators, Mech.\ Syst.\ Signal Process.\ 120 (2019)
808--835.
...
Mater.\ Process.\ Technol.\ 297 (2021) 117238.
...
(2023).  doi:10.1177/10775463231161389.
\end{thebibliography}

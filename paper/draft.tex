...
$49.5$\,mm/s), quantifying the residual $7\%$ as the measured price
of delay independence on this plant.  Every number is derived from
...
($0.232$--$0.656$\,mm), and the open loop ($0.039$\,mm worst)---all
lifted from the open-loop floor by roughly an order of magnitude.  In
the time domain at the reference condition the open loop diverges
within $0.089$\,s, while the members hold the response at
$3$--$4\,\mu$m with peak actuation of $16$--$25$\,V---far inside the
$450$\,V authority.
...
the lobe-first champion of Section~\ref{sec:lobefirst}.  The dotted
line marks the design speed.  Source: \texttt{lobes\_l2.npz}.}
\label{fig:lobes}
\end{figure}
...
$t = 0.089$\,s (dashed marker); all three members hold a
$3$--$4\,\mu$m peak and settle near $1\,\mu$m.  Right: rms actuation
voltage of the three members.
...
Table~\ref{tab:lobefirst} carries the champion's full battery.  It
passes the gate ($0.332$--$0.450$), improves the shared objective it
was not optimised for ($J = -0.166$ vs $-0.281$), raises the nominal
floor to $0.539$\,mm, keeps the $3.0\times$ faster scenario recovery
(at $0.70\times$ the energy), and its whole-pass certificate at
$\ap = 0.15$\,mm reads $v^{*} = 49.5$\,mm/s on all three $a_4$
slices at the \emph{fixed} base rate $\alpha = 10$\,s$^{-1}$---double
the incumbent's $\alpha$-optimised $23.6$--$24.7$\,mm/s, so the
lobe-first push strengthened the certificate rather than spending
it.

\begin{table}[t]
\centering\small
\setlength{\tabcolsep}{5pt}
\caption{The lobe-first round within the filtered class.  Lobe rows
are full-resolution verdicts (Floquet $m = 120$, $3600$--$7200$\,rpm,
min over the tool-path positions); gate and recovery by the standing
machinery, each recovery ratio within one measurement batch.
Sources: \texttt{lobes\_l2.npz}, \texttt{mu\_real\_l.npz},
\texttt{s1\_l.npz}, \texttt{s1\_fresh.npz}, \texttt{cert\_l.npz}.}
\label{tab:lobefirst}
\begin{tabular}{@{}p{4.3cm}p{3.2cm}p{3.3cm}p{3.6cm}@{}}
\toprule
 & $\mu$-TDC (benchmark) & PS-AC-RFA r5 & PS-AC-RFA-L \\
\midrule
...
(\mbox{$\alpha$-opt}), $\ap = 0.15$\,mm & $49.5$\,mm/s
(fixed $\alpha$), $\ap = 0.15$\,mm \\
\bottomrule
\end{tabular}
\end{table}
...
(7)~The lobe-first champion's certificate is reported at the fixed
base rate $\alpha = 10$\,s$^{-1}$; the incumbents' per-slice $\alpha$
optimisation can only raise it.  Its crossing pair
($\ap^{\infty}, \delta_{\max}$) has not been re-measured; the
frozen-speed lobe verdicts of Table~\ref{tab:lobefirst} are the
sharper statement of the same axis.
...
\texttt{lobe\_champions.pkl}, run logs), and corrections to earlier

...
contract by design.  Every number is derived from stored,
re-checkable certificate matrices.
\end{abstract}
...
\subsection*{Structure comparison under the shared protocol}

With the theory fixed, Table~\ref{tab:structures} compares the
controller \emph{structures} under the identical fairness protocol
(same optimiser, seeds, constraints and evaluation;
Section~\ref{sec:plant}), on the three axes the paper certifies:
shared objective $J$, the mixed-$\mu$ gate (actuator block included),
and the whole-pass instruments.  Three structural facts emerge.
First, \emph{the real-reading gate barrier is structural, not a
scheduling artifact}: the fixed LQG fails at $3.84$ almost exactly
where its scheduled counterpart fails ($3.76$--$3.79$), and the fixed
FOPID fails at $3.20$---unfiltered high-gain structures hit the
actuator band regardless of scheduling, while all three
actuator-aware members pass ($0.24$--$0.69$).  Second, \emph{the
robustness--performance price is quantified across structures}:
gate-passing members pay $J \in [-0.48, -0.03]$ against
$J \in [+0.06, +0.15]$ for the gate-failing ones.  Third, \emph{the
realizability pathology has a certification-side twin}: the fixed
LQG's parasitic ${\sim}2$\,MHz Kalman poles leave its pointwise disc
margin huge ($0.05$) and a certificate guaranteed to exist by
Proposition~\ref{prop:kyp}, yet no floating-point construction we
tried can exhibit one (the ${\sim}4{\times}10^{3}$ dynamic range of
the closed loop drives the Riccati conditioning past float64
verification floors)---the same poles that block hardware block the
certificate numerics, while the parasitic-free FOPID certifies at
$47.0$\,mm/s.  Note the two whole-pass columns answer different
contracts: the chain certifies the parametric box and every delay
$\tau' \le \tau$ (no actuator block), the gate adds the actuator
uncertainty; FOPID passes the first and fails the second.

\begin{table}[t]
\centering\small
\caption{Controller structures under the shared protocol.  Gate =
peak mixed $\muRS$ over probed nodes (complex reading in
parentheses); $\ap^{\infty}$ = exact-crossing certified depth (full
box); $\vcert$ = adiabatic whole-pass certificate at
$\ap = 0.10$\,mm ($\dagger$: at $0.15$\,mm).  Sources:
\texttt{stage3\_controllers.pkl}, \texttt{stage56.pkl},
\texttt{mu\_real\_cmp.npz}, \texttt{adiab\_cmp.npz}.}
\label{tab:structures}
\begin{tabular}{@{}lrrrllr@{}}
\toprule
Structure (params) & $J$ & $M_s$ & V/N &
Gate $\muRS$ & $\ap^{\infty}$ (mm) & $\vcert$ (mm/s) \\
\midrule
FOPID, fixed (5) & $+0.057$ & $1.99$ & $240$ &
$3.20$ ($72$) fail & $0.370$ & $47.0$ \\
LQG, fixed (4) & $+0.119$ & $1.99$ & $275$ &
$3.84$ ($87$) fail & $0.382$ & uncert.\ (2\,MHz poles) \\
LQG, scheduled, stored (5) & $+0.145$ & $1.99$ & $272$ &
$3.76$--$3.79$ fail & $0.382$ & uncert.\ (8.8\,MHz poles) \\
$\mu$-TDC (benchmark) (6) & $-0.025$ & $2.00$ & $152$ &
$0.242$--$0.419$ pass & $0.436$ & out of DI contract (TDC) \\
PS-AC-RI, realizable (5) & $-0.482$ & $1.82$ & $109$ &
$0.69$ pass & --- & $79.9$--$82.7$ \\
PS-AC-RFA r5 (8) & $-0.281$ & $1.68$ & $151$ &
$0.283$--$0.383$ pass & $0.284$ & $23.6$--$24.7^{\dagger}$ \\
\bottomrule
\end{tabular}
\end{table}  Under the corrected
...
\texttt{mu\_real\_cmp.npz}, \texttt{adiab\_cmp.npz}, run logs), and corrections to earlier

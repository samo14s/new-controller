...
($0.9307$ mm، $+9.3\%$)، و**أول $\mu_{RS} < 1$** — على الفيزياء البارامترية والموضعُ جدولةً
...
| **الشهادات** | العبور و$a_p^\infty$ و$\delta_{\max}$ على منظومة التصميم بـ**نمطين** |
| المحاكاة | $n_{sub} = 656$ خطوة لكل دور سنّ، تشبّع $\pm450$ V |

**رتبتان لا واحدة.** الشهادات تُحسَب على نموذج التصميم بنمطين، والحدود الزمنية على
نموذج التقييم بخمسة. الفرق مقيس ومُبلَّغ (‏`results/conservatism_budget.txt`، الدرجة
‏R3m→R4): يكاد ينعدم لعائلة LQG (‏−4.4 % و−4.5 % من الفجوة اللوغاريتمية) ويبلغ
‏−162.5 % لـμ‑TDC وحده، لأنّ زوجه المتأخّر يزعزع النمط الثالث الذي لا تراه منظومة
النمطين. فالمقارنات بين الشهادة والقياس تُقرأ بهذا القيد.
...
**خطوة الزمن.** عند $n_{sub} = 164$ (‏40 kHz، نايكويست $20.1$ kHz) تتباعد **خمسة من
ستّة** تصاميم زمنيًّا؛ الناجي الوحيد هو μ‑TDC، وهو التصميم الوحيد الذي يقع أسرع أقطابه
($8.00$ kHz) **تحت** ذلك النايكويست. أمّا البقيّة فأقطابها فوقه: FOPID عند $99.9$ kHz
(‏زاوية Oustaloup العليا)، وعائلة LQG أسرع بكثير بحكم بنائها — من $759$ kHz إلى
$2018$ kHz. وعند $n_{sub} = 328$ فأعلى لا يتباعد أيٌّ منها. فالعلّة ليست Oustaloup
وحدها كما قد يُظنّ، بل **معدَّل الدمج قياسًا بأسرع قطب في التحقيق**؛ وهذا خطأ عدديّ لا
فيزيائي، ويُذكَر لأنّ كل مقاييس التردّد تبدو سليمة بينما الدمج يتباعد
(‏`results/timestep_trap.txt`).
...
**أين تذهب النسبة الباقية.** الـ45 % ليست رقمًا غامضًا: سُلَّمٌ من سبع درجات يغيّر
عند كل درجة قرارَ نمذجةٍ واحدًا، ففروقُ اللوغاريتمات جَمعية وتجمع إلى الفجوة الكلّية
بالضبط (البقيّة $0.00\text{e}{+}00$ — `results/conservatism_budget.txt`):

| السبب | ‏μ-TDC | PS‑AC | PS‑TDC‑R |
|---|---:|---:|---:|
| التغطية البارامترية | ‏+0.0 % | ‏+12.2 % | ‏+9.9 % |
| **غلاف $\alpha_4$** | **+143.1 %** | **+75.2 %** | **+73.2 %** |
| **مستوى $\alpha_4$** | **+113.6 %** | **+60.0 %** | **+53.6 %** |
| الاستقلال عن التأخير | ‏+0.0 % | ‏+0.0 % | ‏+0.0 % |
| نموذج التصميم 2→5 أنماط | ‏−162.5 % | ‏−4.4 % | ‏−4.5 % |
| التجميد مقابل الدوريّة | ‏+5.8 % | ‏−43.0 % | ‏−32.2 % |

وثلاث قراءات تُغيّر ما ينبغي فعله بعد هذه الورقة. **الاستقلال عن التأخير مجّانيّ**:
‏$0.0\ \%$ عند الثلاثة، لأنّ ترددات العبور تسكن الأنماط البنيوية فأوّل تأخير عبور
أقصر من $\tau_0$ سلفًا — فلا شيء يُدفَع ثمنًا لطلب «كل تأخير» بدل التأخير الفعليّ،
وترقيةُ **اختبار** التأخير لا تشتري شيئًا هنا. و**التحفّظ كلّه في معالجة
$\alpha_4$**: غلافُها ومستواها معًا يحملان $127$–$257\ \%$ من الفجوة، أي أنّ المسألة
في **المجموعة** لا في الاختبار. وأخيرًا **التجميد ليس دائمًا في جانب الأمان**:
الدرجة الأخيرة سالبة لـPS‑AC وPS‑TDC‑R، أي أنّ تجميد $\alpha_4$ على متوسّطها
يُبالغ في التفاؤل بشأن الدوريّة، ويُذكَر ذلك كما هو.

وأصدق صياغةٍ للاختبار المجمَّد تكشف حدًّا أعمق. المعادلة (23) تجمّد $\alpha_4$ على
$[0.3,\,2.9]\bar\alpha_4$، بينما $\alpha_4(t)$ الحقيقية تبلغ $12.43\bar\alpha_4$
وتقضي $1.05\ \%$ فقط من دور السنّ داخل ذلك النطاق و$10.40\ \%$ فوقه. وحين يُعاد
حساب $a_p^\infty$ على الغلاف الحقيقي (`results/envelope_probe.txt`) يعطي
**‏0.0967 mm للتصاميم الثلاثة جميعًا** مقابل $0.0030$ mm للحلقة المفتوحة: التغذية
الراجعة تبقى تساوي $32.5\times$، لكنّ الفارق بين التصاميم يصير $0.000\ \%$.
فالاختبار المجمَّد على غلافه الصادق **لا يميّز** بين المرشّحين — وهذا، لا التحفّظ
وحده، هو ما يستدعي مجموعةً تعرف دورة عمل $\alpha_4(t)$ بدل طرفها.
...
التصميم والشهادات)، لكنّ المكسب الزمنيّ حقيقيّ: أرضيةٌ تعادل LQG ($0.8459$) وأسوأ
حالة $0.6616$ **بلا انهيار** تحت صندوق المرجع.

> **سُحِب:** كانت هذه الفقرة تنسب إلى PS‑AC‑R شهادةَ Lyapunov–Krasovskii بـ$P$
> مشتركة. تدقيقٌ لاحق (§9.5) بيّن أنّ عمود LK كلّه أثرُ حلّالٍ لا شهادة، فحُذف
> الادّعاء من هنا ومن كل موضع.

### 8.4 التركيب: PS‑TDC‑R

الزوج المتأخّر فوق القاعدة الغلافية — سبعة بارامترات مضبوطة، واحد فوق
ستّة μ-TDC، ويُبلَّغ كذلك. يهبط PSO في حوض الإلغاء نفسه ($k_{pd} = -1.495$،
$k_{dd} = +0.546$) عند $J = +0.174$، الأعلى في الدراسة كلّها:

| | بار. | $a_p^\infty$ | $\delta_{\max}$ | الأرضية | أسوأ-4 | صندوق $+10\%$ |
|---|---|---|---|---|---|---|
| ‏μ-TDC [1] | 6 | **0.4364** | **1.54** | 0.6821 | 0.442 | ✓ |
| PS‑AC | 4 | 0.3836 | 1.40 | 0.8517 | 0.6528 | ✓ |
| **PS‑TDC‑R** | 7 | 0.3954 | 1.43 | **0.9307** | 0.6704 | ✓ |

‏**3/5**: أفضل أرضية في الدراسة ($+9.3\%$ على PS‑AC و$+36\%$ على μ-TDC)، وأسوأ حالة
فوق PS‑AC، ولا انهيار. والمعياران الباقيان (هامشا العبور) يبقيان لـμ-TDC بدليل
الاستحالة أعلاه.

> **سُحِب:** كان هذا الجدول يحمل عمودًا سابعًا لشهادة LK، وكانت الحصيلة تُقرأ
> «3/5 مع شهادة LK». حُذف العمود والعبارة بعد تدقيق §9.5. والحصيلة العددية
> ‏3/5 لم تتغيّر: المعايير الخمسة المُعلَنة سلفًا لم تتضمّن LK أصلًا.
...
### 9.5 عمود Lyapunov–Krasovskii — تصحيحٌ بالسحب

كانت أحكام LK في هذه الدراسة تُقرأ من **حالة الحلّال** وإشارة متغيّر الفسحة، دون
النظر إلى المصفوفات العائدة. وهذا بالضبط ما لا يجوز لشهادة. وإعادة الحلّ لكل تصميم
عند $a_p^\infty$ المخزون، ثم التحقّق من $(P,Q)$ العائدة مقابل المتباينات نفسها
(‏`results/lk_audit.txt`)، تعطي:

- **صفرٌ من اثني عشر** تصميمًا يعيد مصفوفاتٍ تُحقّق $P \succ 0$ و$Q \succ 0$
  و$\Psi \prec 0$ عند كل رأس. والانتهاكات فادحة لا حدّية: أصغر قيمة ذاتية نسبية
  لـ$P$ تصل إلى $-2.13$، وأكبر قيمة ذاتية نسبية لـ$\Psi$ موجبة في الاثني عشر.
- **ثلاثة من اثني عشر** تختلف عن الحكم المخزون عند إعادة التشغيل — من بينها
  ‏PS‑AC نفسه، الذي بُنيت عليه مقارنة §8.3. فالعمود لا يعيد إنتاج أحكامه.
- الموازنة القُطرية للحزمة (تكافؤٌ تامّ حين يكون $P$ مشتركًا) تحسّن البواقي ولا
  تشفيها؛ والسبب المرجَّح شرطٌ عدديّ رديء: الحلقة المغلقة تمتدّ نحو $4000\times$
  في التردّد، إذ تعلو أقطاب التحقيق إلى $0.76$–$2.0$ MHz فوق لوحةٍ عند $540$ Hz.

فالعمود **مسحوب** من الملخّص و§8.3 و§8.4 و§10.2، و`lk_lmi.certificate_di` صار
يتحقّق من مخرجه قبل أن يسمّيه مُجديًا. ولا يمسّ السحبُ نتيجةً أخرى: محور التأخير
يحمله اختبار العبور، وهو **ضروريّ وكافٍ** عند كل رأس مجمَّد، ولا يمرّ بأي SDP.
...
- تحليل العبور يُجرى على $\alpha_4$ مجمَّدة عند طرفَي مجالها؛ النظام الحقيقي دوريّ بنسبة
  ذروة/متوسط $12.4$.
- **لا شهادة Lyapunov–Krasovskii في هذه الدراسة** (§9.5). العمود الذي كان يحملها
  مسحوب: صفرٌ من اثني عشر تصميمًا يعيد مصفوفاتٍ تُحقّق المتباينات فعلًا. فالتغطية
  الإضافية التي كانت تُنسَب إليه — الاستقرار تحت تغيّر زمنيّ سريع كيفما كان —
  **لم يَنَلها أيُّ متحكّم هنا**، وتبقى مسألةً مفتوحة. ومحور التأخير غير متأثّر:
  يحمله اختبار العبور وحده، وهو ضروريّ وكافٍ عند كل رأس مجمَّد.
